...
printf "\033[36m[  INFOS  ]\033[0m Step 5 - building the system\n"

#
# Preparing the build
#

printf "\033[36m[  INFOS  ]\033[0m checking for file requirements...\n"
...
mkdir -p $dir_system_log													&& \
	printf "\033[32m[  INFOS  ]\033[0m log directory created\n"				|| \
	printf "\033[31m[  INFOS  ]\033[0m failed to create log directory\n"

#
# Build the system
#

printf "\033[36m[  INFOS  ]\033[0m ready to build system\n"

for line in `cat $dir_system_config`
do
	build_file=$(echo $dir_system/$line.sh)
	printf "\033[36m[  INFOS  ]\033[0m running %-20s" "$build_file"
	sh $build_file "/sources" "/logs"
	status=$?
	if [ $status -eq 0 ]; then	
		printf "\033[32mV\033[0m\n"
	else
		printf "\033[31mX (%d)\033[0m\n" "$status"
		exit $status
	fi
done
